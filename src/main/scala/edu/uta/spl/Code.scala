  ...
  def code(e: Expr, level: Int, fname: String): IRexp =
    e match {
      ...
      case UnOpExp(op, right) =>
        val processedRight = code(right, level, fname)
        val uppercaseOp = op.toUpperCase()
        Unop(uppercaseOp, processedRight)
      ...
      case ArrayExp(exprs) =>
        val arrayVar = allocate_variable(new_name("array"), typechecker.typecheck(exprs.head), fname)
        ESeq(Seq(List(
          Move(arrayVar, Allocate(IntValue(exprs.length + 1))),
          Move(Mem(arrayVar), IntValue(exprs.length))) ::: exprs.zipWithIndex.map {
          case (e, i) =>
            Move(Mem(Binop("PLUS", arrayVar, IntValue((i + 1) * 4))), code(e, level, fname))
        }), arrayVar)
      ...
      case CallExp(name, args) =>
        ...
        var diff = level - l
        var staticLink: IRexp = if (l == level + 1) {
          Reg("fp")
        } else if (diff > 0) {
          var link = Mem(Binop("PLUS", Reg("fp"), IntValue(-8)))
          while (diff > 1) {
        ...
        Call(flabel, staticLink, args.map(a => code(a, level, fname)))

      case RecordExp(components) =>
        val typ = components.map {
          case Bind(l, r) => Bind(l, typechecker.typecheck(r))
        }
        val recordVar = allocate_variable(new_name("rec"), RecordType(typ), fname)
        ESeq(Seq(List(Move(recordVar, Allocate(IntValue(components.length)))) ::: components.zipWithIndex.map {
          case (Bind(l, r), i) => Move(Mem(Binop("PLUS", recordVar, IntValue(i * 4))), code(r, level, fname))
        }), recordVar)
  ...
  def code(e: Stmt, level: Int, fname: String, exit_label: String): IRstmt =
    e match {
      ...
      case WhileSt(condition, body) =>
        ...
        Seq(List(
          Label(loopLabel),
          CJump(Unop("NOT", code(condition, level, fname)), exitLabel),
          code(body, level, fname, exitLabel),
          Jump(loopLabel),
          Label(exitLabel)))

      case IfSt(condition, then_stmt, else_stmt) =>
        val thenLabel = new_name("cont")
        val exitLabel = new_name("exit")
        val processedCondition = code(condition, level, fname)
        val processedThen = code(then_stmt, level, fname, exitLabel)
        var processedElse: IRstmt = Seq(List())
        if (else_stmt != null) {
          processedElse = code(else_stmt, level, fname, exit_label)
        }
        Seq(List(
          CJump(processedCondition, exitLabel),
          processedElse,
          Jump(thenLabel),
          Label(exitLabel),
          processedThen,
      ...
      case CallSt(name, args) =>
        ...
        var diff = level - l
        val staticLink: IRexp = if (l == level + 1) {
          Reg("fp")
        } else if (diff > 0) {
          var link = Mem(Binop("PLUS", Reg("fp"), IntValue(-8)))
          while (diff > 0) {
        ...
        CallP(flabel, staticLink, args.map(a => code(a, level, fname)))
